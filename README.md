...
CREATE OR REPLACE TASK PLATFORM_DEV.DBT_DEPLOYMENT.SNOWFLAKE_DBT
  WAREHOUSE = WH_DEV_DBT
  SCHEDULE = 'USING CRON 1 * * * * Europe/Warsaw'
AS
  EXECUTE DBT PROJECT SNOWFLAKE_DBT ARGS = 'run --target dev';

ALTER TASK PLATFORM_DEV.DBT_DEPLOYMENT.SNOWFLAKE_DBT RESUME;
```

Optional test run:

```sql
EXECUTE TASK PLATFORM_DEV.DBT_DEPLOYMENT.SNOWFLAKE_DBT;
```
